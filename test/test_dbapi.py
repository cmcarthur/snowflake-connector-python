...
def _paraminsert(cur):
    ...
    dbapi_ddl2s.sort()
    assert dbapi_ddl2s[0] == "Cooper's", (
        'cursor.fetchall retrieved incorrect data')
    assert dbapi_ddl2s[1] == "string inserted into table", (
        'cursor.fetchall retrieved incorrect data')
...
def test_rowcount(conn_local):
    with conn_local() as con:
        ...
        cur.execute(("insert into %s values "
                     "('string inserted into table')") % TABLE1)
        cur.execute("select name from %s" % TABLE1)
        assert cur.rowcount == 1, (
            'cursor.rowcount should the number of rows returned'
        )
...
def test_executemany(conn_local):
    with conn_local() as con:
        ...
        cur.executemany(
            'insert into %s values (%%(dbapi_ddl2)s)' % (
                TABLE1
            ),
            margs
        )
        ...
        dbapi_ddl2s = [res[0][0], res[1][0]]
        dbapi_ddl2s.sort()
        assert dbapi_ddl2s[0] == "Boag's", (
            'incorrect data retrieved')
        assert dbapi_ddl2s[1] == "Cooper's", (
            'incorrect data retrieved')
...
def test_fetchone(conn_local):
    with conn_local() as con:
        ...
        cur.execute(
            "insert into %s values ('Row 1'),('Row 2')" % TABLE1)
        cur.execute(
            'select name from %s order by 1' % TABLE1)
        r = cur.fetchone()
        assert len(r) == 1, (
...
def test_setinputsizes(
    ...
    with conn_local() as con:
        ...
        _paraminsert(
            cur)  # Make sure cursor still works


def test_setoutputsize_basic(
        conn_local):
    # Basic test is to make sure setoutputsize doesn't blow up
    with conn_local() as con:
        cur = con.cursor()
        cur.setoutputsize(1000)
        cur.setoutputsize(2000,
                          0)
        _paraminsert(
            cur)  # Make sure the cursor still works
...
def test_None(conn_local):
    with conn_local() as con:
        ...
        executeDDL1(cur)
        cur.execute(
            'insert into %s values (NULL)' % TABLE1)
        cur.execute(
            'select name from %s' % TABLE1)
        r = cur.fetchall()
        assert len(r) == 1
...
def test_substring(
    ...
    with conn_local() as con:
        ...
        args = {
            'dbapi_ddl2': '"" \"\'\",\\"\\"\"\'\"'}
        cur.execute(
            'insert into %s values (%%(dbapi_ddl2)s)' % TABLE1,
            args)
        cur.execute(
            'select name from %s' % TABLE1)
        res = cur.fetchall()
